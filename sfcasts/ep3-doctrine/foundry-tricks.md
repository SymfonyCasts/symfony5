...
to `-1 day`. And because this is more flexible, we can even change it from
...
`self::faker()->realText()` to get several words of "real looking" text. For
`slug`, there's a feature for that! `self::faker()->slug`. Finally, for the
question text, it can be made much more interesting by using
...
`$question->getSlug()` - in case we set it manually for some reason - then
use Symfony's Slugger - `$slugger = new AsciiSlugger()` - and set
it with `$question->setSlug($slugger->slug($question->getName()))`.
