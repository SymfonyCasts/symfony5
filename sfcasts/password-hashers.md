...
password. This has been renamed to `password_hashers`. And instead of needing our
...
And since *every* user class with a password will implement this - including our
class - this covers us.

Oh, bug if you had a *different* algorithm before, move that down to this line.
We don't want to *change* algorithms, just delete `encoders` in favor of
`password_hashers`.
...
Let's swing over and fix these. In `templates/base.html.twig`... yep. When we upgraded
the `twig-bundle` recipe, this conflicted and I didn't even notice.
...
By the way, speaking of "security" and "firewalls", Symfony ships with a new
...
It will tell you your firewall names: `dev` and `main`. Re-run this with `main`:
...
Ok, put a big check mark next to "upgrade security". Next, let's *crush* the last
few deprecations and learn how we can be sure that we didn't miss any.
